    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            for selector in image_selectors:
                try:
                    image_btn = await page.wait_for_selector(selector, timeout=3000)
                    if image_btn:
                        print(f"  [成功] 找到图文选项: {selector}")
                        break
                except:
                    continue
                    ...
                    try:
                        ...
                        info = await card.evaluate('''(card) => {
                            ...
                            for (const span of allSpans) {
                                ...
                                // 找第一个有实质内容的文本作为描述
                                if (text.length > 10 && !text.includes('#') && !text.includes('展开') && !text.includes('·')) {
                            ...
                            // 提取作者
                            const authorEl = card.querySelector('[class*="TLKbIZlL"], [class*="arnSiSbK"]');
                            if (authorEl) {
                                result.author = authorEl.textContent.trim();
                            } else {
                                // 备用方案：查找第一个链接的文本
                                const firstLink = card.querySelector('a[href*="/user/"]');
                                if (firstLink) {
                                    result.author = firstLink.textContent.trim();
                                } else {
                                    result.author = '未知作者';
                                }
                            }
                            ...
                            if (timeEl) {
                                result.timeText = timeEl.textContent.trim().replace('·', '').trim();
                            } else {
                                result.timeText = '';
                            }
                            ...
                            if (likeEl) {
                                result.likesText = likeEl.textContent.trim();
                            } else {
                                // 备用：查找包含数字的元素
                                const allDivs = card.querySelectorAll('div');
                                for (const div of allDivs) {
                                    const text = div.textContent.trim();
                                    if (/^\d+$/.test(text) || /^\d+\.?\d*[万w]?$/.test(text)) {
                                        result.likesText = text;
                                        break;
                                    }
                                }
                                if (!result.likesText) {
                        ...
                        # 生成唯一ID（避免UNIQUE约束冲突）
                        import time
                        unique_id = f"{keyword}_{int(time.time()*1000)}_{random.randint(1000,9999)}"
